...
- **Route Protection**: Middleware at `middleware.ts` blocks direct navigation to protected pages (`/vote`, `/enroll`, `/consent`, `/history`) and redirects unauthenticated users to `/auth`. Update the middleware to validate JWT/session once implemented.
